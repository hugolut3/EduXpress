            ...
            try
            {
               ...
               // string currentdateTime = DateTime.Now.ToString(CultureInfo.CurrentCulture.DateTimeFormat.FullDateTimePattern, CultureInfo.CurrentCulture);
                    ...
                    if (rdr.Read())
                    {
                        ...
                        if (addTimeBulletinGlobalVariable == true)
                        {
                            ...
                            if (townCommune != "") 
                            {
                                ...
                                xrLblFooterText.Text = LocRM.GetString("strDoneAt") + " " + townCommune + ", " + LocRM.GetString("strThe") + " " + reportDateGlobalVariable;
                            }
                            else
                            {
                                // xrLblFooterText.Text = LocRM.GetString("strDate") + " " + myTI.ToTitleCase(currentdateTime);
                                xrLblFooterText.Text = LocRM.GetString("strDate") + " " + reportDateGlobalVariable;
                            }
                        }
                        else
                        {
                            xrLblFooterText.Text = LocRM.GetString("strDoneAt") + "  ................        " + LocRM.GetString("strThe") + "  .............";
                        }
                ...
                using (con = new SqlConnection(databaseConnectionString))
                {
                    ...
                    if (rdr.Read())
                    {
                        xrLblStudent.Text = LocRM.GetString("strStudent").ToUpper() + ": " + rdr.GetString(1).ToUpper() + " " + rdr.GetString(2).ToUpper();
                        string sexe = rdr.GetString(3);
                        if (sexe == "Garçon")
                        {
                            xrLblSexe.Text = LocRM.GetString("strSex").ToUpper() + ": M";
                        }
                        else if (sexe == "Fille")
                        {
                            xrLblSexe.Text = LocRM.GetString("strSex").ToUpper() + ": F";
                        }
