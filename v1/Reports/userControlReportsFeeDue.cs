...
namespace EduXpress.Reports
{
    public partial class userControlReportsFeeDue : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void AutocompleteAcademicYear()
        {
            ...
            try
            {
                ...
                using (con = new SqlConnection(databaseConnectionString))
                {
                    ...
                    if (cmbSchoolYearPartial.Properties.Items.Count > 0)
                    {

                        currentYearComboIndex = cmbSchoolYearPartial.Properties.Items.Count - 1;
                        cmbSchoolYearPartial.SelectedIndex = currentYearComboIndex; //Select current school year.
                    }
        ...
        {
            cmbAmountOwedStudent.Properties.Items.Clear();
            cmbAmountOwedStudent.Properties.Items.AddRange(new object[]
                {
                    LocRM.GetString("strFeesDueFull"),
                    LocRM.GetString("strFeesDuePartiallyAnyAmount") ,
                    LocRM.GetString("strFeesDuePartiallyPercentageAmount")                    
                });
        }
        ...
        {
            cmbSchoolYearPartial.SelectedIndex = currentYearComboIndex;
            ...
            cmbAmountOwedStudent.SelectedIndex = -1;
            trackBarPercentage.EditValue = trackBarPercentage.Properties.Minimum;
        }
        ...
        private void btnSearchPartialDueList_Click(object sender, EventArgs e)
        {
            ...
            if (cmbAmountOwedStudent.Text == "")
            {
                XtraMessageBox.Show(LocRM.GetString("strSelectCategory"), LocRM.GetString("strInputError"), MessageBoxButtons.OK, MessageBoxIcon.Error);
                cmbAmountOwedStudent.Focus();
                return;
            }

            if (cmbAmountOwedStudent.SelectedIndex == 0)  //Full amount
            {
                SearchFullDueList();
            }
            else if (cmbAmountOwedStudent.SelectedIndex == 1) //Any amount
            {
                SearchPartialDueListAnyAmount();
            }

            else if (cmbAmountOwedStudent.SelectedIndex == 2) //Percentage amount
            {
                SearchPartialDueListPercentageAmount();
            }

        }

        private void SearchFullDueList()
        {            
            try
            {
                //clear gridcontrol
                ...
                using (con = new SqlConnection(databaseConnectionString))
                {
                    ...
                    cmd.Connection = con;
                    cmd.Parameters.AddWithValue("@d1", cmbSchoolYearPartial.Text.Trim());
                    cmd.Parameters.AddWithValue("@d2", cmbClassPartial.Text.Trim());
                    cmd.Parameters.AddWithValue("@d3", cmbMonthPartial.Text.Trim());
        ...
        private void SearchPartialDueListAnyAmount()
        {
            try
            {
                ...
                PublicVariables.IsFeesDuePercentage = false;
            }
            catch (Exception ex)
            {
                if (splashScreenManager1.IsSplashFormVisible == true)
        ...
        private void SearchPartialDueListPercentageAmount()
        {
            try
            {
                ////*****************this code to be optimized*********** check totalFeePaid first***************************//
                ...
                //decimal TotalFeePerPeriod = 0;
                // string sTotalFeePerPeriod = "";
                using (con = new SqlConnection(databaseConnectionString))
                {
                    con.Open();

                    cmd = new SqlCommand("SELECT SUM( distinct CourseFeePayment.TotalPaid),StudentNumber,TotalFee from CourseFeePayment INNER JOIN CourseFeePayment_Join on CourseFeePayment.CourseFeePaymentID=CourseFeePayment_Join.C_PaymentID where Month=@d3 and CourseFeePayment.SchoolYear=@d1 and CourseFeePayment.Student_Class=@d2 group by StudentNumber,Month,TotalFee", con);
                    cmd.Parameters.AddWithValue("@d1", cmbSchoolYearPartial.Text);
                    cmd.Parameters.AddWithValue("@d2", cmbClassPartial.Text);
                    cmd.Parameters.AddWithValue("@d3", cmbMonthPartial.Text);
                    ...
                    ArrayList sTotalPaidFees = new ArrayList(ds.Tables[0].Rows.Count);
                    //ArrayList sStudentNumbers = new ArrayList(ds.Tables[0].Rows.Count);
                    foreach (DataRow dRow in dtable.Rows)
                    {
                        sTotalPaidFees.Add(dRow[0].ToString());  //Add total Paid for each student in the array
                                                                 // sTotalFeePerPeriod = dRow[2].ToString();  //total fee per month
                                                                 // sStudentNumbers.Add(dRow[1].ToString());  //Student number of student
                    }
                    totalPaidFees = (string[])sTotalPaidFees.ToArray(typeof(string));
                    //TotalFeePerPeriod = decimal.Parse(sTotalFeePerPeriod);
                    // StudentNumbers = (string[])sStudentNumbers.ToArray(typeof(string));// Add the student number list to array
                    con.Close();
                }
                ...
                using (con = new SqlConnection(databaseConnectionString))
                {
                    ...
                    ct = "SELECT distinct RTRIM(Students.StudentNumber),RTRIM(Students.StudentSurname),RTRIM(Students.StudentFirstNames),RTRIM(Students.NotificationNo),RTRIM(Students.NotificationEmail),RTRIM(TotalFee) from Students, CourseFeePayment, CourseFeePayment_Join where Students.StudentNumber = CourseFeePayment.StudentNumber and CourseFeePayment.CourseFeePaymentID = CourseFeePayment_Join.C_PaymentID and CourseFeePayment.SchoolYear = @d1 and CourseFeePayment.Student_Class = @d2  and Month = @d3 and PaymentDue > 0  order by 2"; // add notification no and email
                    cmd = new SqlCommand(ct);
                    cmd.Connection = con;
                    cmd.Parameters.AddWithValue("@d1", cmbSchoolYearPartial.Text);
                    cmd.Parameters.AddWithValue("@d2", cmbClassPartial.Text);
                    ...
                    {
                        totalFee = decimal.Parse(rdr[5].ToString(), CultureInfo.InvariantCulture);
                        decimal duePercentage = (totalFee * (int)trackBarPercentage.EditValue) / 100;
                        //new addition to dispolay  percentage of amount due

                        //feeDue = decimal.Parse(totalPaidFees[rowNumber], CultureInfo.InvariantCulture);
                        feeDue = totalFee - decimal.Parse(totalPaidFees[rowNumber]);
                        string sFeeDue = feeDue.ToString("0.00");

                        if (feeDue > duePercentage)  //Check if this student owes more than this percentage amount
                        {
                            //add new product row
                            gridView1.AddNewRow();
                            gridView1.UpdateCurrentRow();
                ...
                PublicVariables.FeePeriod = cmbMonthPartial.Text;
                PublicVariables.FeesDueFull = false;
                PublicVariables.IsFeesDuePercentage = true;
                PublicVariables.PercentageFeesDue = (int)trackBarPercentage.EditValue;
            }
            catch (Exception ex)
            {
                if (splashScreenManager1.IsSplashFormVisible == true)
        ...
        */
        ...
        private void getPhoneNumbers()
        {
            ...
            else
            {
                ...
                btnSearchPartialDueList.Focus();
                return;
            }
                
        }

        private void cmbAmountOwedStudent_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbAmountOwedStudent.SelectedIndex ==2)
            {
                trackBarPercentage.Visible = true;
            }
            else
            {
                trackBarPercentage.Visible = false;
                trackBarPercentage.EditValue = trackBarPercentage.Properties.Minimum;
            }
        }
    }
