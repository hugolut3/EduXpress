...
{    
    public partial class userControlMarkSheet : DevExpress.XtraEditors.XtraUserControl, Functions.IMergeRibbons
    {
                ...
                {                    
                    ...
                    using (con = new SqlConnection(databaseConnectionString))
                    {
                        ...
                        try
                        {
                            ...
                            string cb = "update MarksEntry set Class=@d2,SubjectMaxima=@d6,MarksObtained=@d7 where SchoolYear=@d1 and StudentNumber=@d3 and AssessmentPeriod=@d5 and SubjectCode=@d4";
                            cmd = new SqlCommand(cb, con, transaction);
                            cmd.Connection = con;
        ...
        private void DeleteRecord()
        {
            ...
            //    {
            //        con.Open();
            //        string cq = "delete from MarksEntry where SchoolYear=@d1 and Class=@d2 and StudentNumber=@d3 and AssessmentPeriod=@d5 and SubjectCode=@d4";
            //        cmd = new SqlCommand(cq);
            //        cmd.Parameters.AddWithValue("@d1", cmbSchoolYear.Text.Trim());
            //        cmd.Parameters.AddWithValue("@d2", cmbClass.Text.Trim());
            //        cmd.Parameters.AddWithValue("@d3", gridView1.GetRowCellValue(i, LocRM.GetString("strStudentNo").ToUpper()).ToString(); //Loop throuh to delete marks for all students
            //        cmd.Parameters.AddWithValue("@d5", txtID.Text.Trim());
            //        cmd.Parameters.AddWithValue("@d4", txtID.Text.Trim());
            //        cmd.Connection = con;
